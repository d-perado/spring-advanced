...
## 5. 트러블슈팅
### 문제1. 회원가입시에 유효성 검사를 하지않는 점

[문제 인식 및 정의]
회원가입시에 유효성 검사를 하지않으면 DB의 일관성이 떨어짐.
업데이트에서 유효성 검사를 해도 DB에는 유효성 검사를 하지 않은 값들이 남기때문에 의미가 없다

[해결 방안]
회원가입시에도 유효성 검사하도록 변경

[해결 완료]
회원가입시에도 유효성검사하도록 변경하여 데이터에 일관성 확보

### 문제2. 사용하지 않는 Response가 존재하는 점.

[문제 인식 및 정의]
사용하지 않는 Response의 존재로 인해 불필요한 파일이 늘어남.
심지어 재활용의 여지도 없음.
[해결 방안]
해당 Response 삭제.

### 문제3.
UserResponse 를 UserDTO처럼 사용하는 점

[문제 인식 및 정의]
Response 의 책임의 영역에서 벗어나서
유지보수에 문제가 생길 수 있다고 판단하였습니다.

[해결 방안]
UserResponse 중
Response의 역할보다
DTO의 역할이 크다고 판단되는것들 변경